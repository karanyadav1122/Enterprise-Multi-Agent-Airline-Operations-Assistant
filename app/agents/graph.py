...
from app.observability.logging import logging
...
def route_request(state: AirlineState):
    ...
    logger.info(
        "request_id=%s user_id=%s intent=%s query=%s",
        request_id,
        user_id,
        intent,
        query,
    ...
    logger.debug(
        "request_id=%s user_id=%s intent=%s event=route_debug",
        request_id,
        user_id,
        intent,
    )
    ...
    if intent == "flight":
        response = flight_agent(query, user_id, request_id)
...
def run_airline_agent(user_query: str, user_id: str, request_id: str) -> str:
    result = graph.invoke(
        {
